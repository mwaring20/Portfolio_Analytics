...
from pillar1.adapters.schwab import load_schwab_export, parse_declared_record_count
from pillar1.validation import validate_transactions_df

FIXTURE = Path(__file__).parent / "fixtures" / "schwab_export_phantom.csv"


def test_declared_record_count_parsed_from_preamble():
    assert parse_declared_record_count(FIXTURE) == 100
