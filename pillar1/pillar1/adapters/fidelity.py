...
from ..type_mapping import PrefixTypeMap, load_fidelity_action_map
...
def _map_action(action: str, action_map: PrefixTypeMap) -> Tuple[str, bool, Union[str, None]]:
    return action_map.resolve(action)
...
def load_fidelity_export(
    path: Union[str, Path], action_map: Optional[PrefixTypeMap] = None
    ...
    action_map: override the config-driven Action-column mapping (see
    type_mapping.py). Defaults to config/fidelity_action_map.yaml.
    """
    action_map = action_map or load_fidelity_action_map()
    path = Path(path)
    ...
    for idx, row in enumerate(data_rows):
        ...
        action = row["Action"]
        txn_type, needs_review, review_reason = _map_action(action, action_map)
